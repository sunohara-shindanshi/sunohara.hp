...
- **Cookie 同意管理は「実装しない」方針**（2026-08-06 決定）
  - 同意バナー・Consent Mode・オプトアウト導線はいずれも実装していません。
    ページを開いた時点で GTM → GA4 が読み込まれ、GA4 の Cookie（`_ga` など）が設定されます。
  - 日本国内向けサイトのため必須ではない、という判断です。
    （日本の電気通信事業法の外部送信規律では、アクセス解析目的の送信について
    「通知・公表」または「同意」等の対応が求められる場合があります。
    必要になった場合は、プライバシーポリシーへの記載（＝公表）が最も軽い対応です。）
  - **EU / 英国からのアクセスも計測対象にする場合は、この判断を見直す必要があります**
    （GDPR では事前同意が必要で、Google も広告連携時は Consent Mode v2 を要求しています）。
  - 実装が必要になった場合の要点：GTM の「同意設定」で GA4 タグに
    `analytics_storage` を必須指定し、既定を `denied` にしたうえで、
    同意取得後に `gtag('consent','update',…)` 相当の更新を送る構成にします。
    このとき、コード側で追加が必要なのは「同意状態を dataLayer に push する処理」だけで、
    既存のイベント送信の仕組みには手を入れません。
