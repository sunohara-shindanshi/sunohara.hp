...
- **グローバルナビ**：sun の「相談する」ボタン＋電話番号（PC・モバイル両方）。
- **ファーストビュー**：ホームはヒーローに CTA＋電話、下層ページは `components/PageHeader.tsx` の `cta` プロップで各ページに CTA＋電話を表示（お問い合わせページは自ページのため電話のみ）。
- **コンテンツ末尾**：`components/CtaSection.tsx`（濃紺の帯・中央揃え）を各ページ末尾に配置。ページごとに見出し・リード文・ボタンラベルを内容に合わせて変えています（例：事業内容→「自社に必要な支援が分からない、から始めてOKです」）。ボタンは主要 CTA 1 つ＋電話番号のみに絞っています。
